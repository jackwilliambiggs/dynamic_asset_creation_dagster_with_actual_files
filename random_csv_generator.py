...
def generate_random_data(num_rows: int) -> pd.DataFrame:
    ...
    for _ in range(num_rows):
        row = {
            ...
            'name': ''.join(random.choices(string.ascii_uppercase + string.ascii_lowercase, k=8)),  # Random name (8 characters)
            'age': random.randint(18, 99),  # Random age between 18 and 99
            'email': ''.join(random.choices(string.ascii_lowercase + string.digits, k=8)) + '@example.com',  # Random email
            ...
            'city': random.choice(['New York', 'Los Angeles', 'Chicago', 'Houston', 'Phoenix', 'Philadelphia', 'San Antonio'])  # Random city
        }
        data.append(row)
